---
description: 기존 코드베이스를 영역 하네스로 수용(adopt 시 소스를 각 영역 content/ 로 물리 이동). 구조 제안 → 승인 → git mv 이동 → 참조 재작성 → 빌드 게이트 → registry 작성.
...
> **승인 전엔 아무것도 옮기지 않는다.** 승인 후 각 말단의 소스를 `git mv` 로 `areas/<path>/content/` 안으로 물리 이동하고, 깨지는 참조를 재작성하고, 빌드/테스트 통과를 게이트로 둔다. 실패하면 전체 롤백한다.
>
> **영역 폴더 = 정확히 두 칸:** `ssot/`(NOTES.md 본문 + `<id>.md` ssot 문서) + `content/`(코드 — 폴더 구조 그대로). 별도 `src/` 층 없음. 코드는 `content/` 직속.
...
3. **공유 코드 검출 (필수 — 건너뛰면 빌드가 깨진다)**: import 그래프를 만들어 **≥2 영역이 import 하는 모듈**을 찾는다.
   ...
   - ≥2 영역이 쓰는 모듈 → 전용 **shared 말단 영역**(`areas/shared/content/`)으로 모으고 소비 영역은 거기서 import(영역 간 import 허용).
...
4. **영역 제안 (가장 중요)** — `GLOBAL.md` 「영역 규칙」을 따른다:
   ...
   - 큰 묶음은 중간다리, 실제 코드 묶음은 말단. 말단마다 어떤 파일/글롭이 `content/` 로 들어갈지 매핑하고, 공유 모듈은 `shared` 로 표시.

5. **사용자 확인**: 제안한 영역 트리 + **이동 계획**(파일 → `areas/<path>/content/...` 쌍, shared 분류 포함)을 보여주고 **승인받는다**. 큰 변경이므로 이동 전에 멈춘다.
...
7. **이동 (`git mv`)**: 각 말단의 소스를 `areas/<path>/content/` 로 옮긴다(폴더 구조 그대로). shared 모듈은 `areas/shared/content/` 로. 모든 이동은 `git mv`(이력 보존). 대상 폴더는 먼저 만든다.
...
10. **하네스 작성**(게이트 통과 후):
    - `registry.json` 갱신(단일 소스). 말단 `path` 는 `areas/<...>/` — 코드 위치는 그 `content/` 로 암묵 고정(밖을 가리키는 `sources` 없음).
    - 각 말단에 `ssot/NOTES.md`(본문 — 코드 링크는 `../content/...` 상대경로) + 필요한 `ssot/<id>.md`(frontmatter `category` 필수). 코드는 `content/`. 중간다리는 `ssot/NOTES.md`(자식 목록)만, `content/` 는 비움.
    - 공유되는 규약은 `_ssot/<id>.md` 정본 + `shared_ssot.shown_in`.

11. `node check-registry.mjs` 실행 → 오류 0 까지. `content/` 가 빈 말단·`ssot/NOTES.md` 누락 경고가 남으면 수용이 끝났는지 재확인.

12. `logs/log.jsonl` 에 append: 영역별 `{"op":"area:add","target":"<id>",...}` + 이동별 `{"op":"code:move","target":"<id>","from":"<원경로>","to":"areas/<path>/content/...","summary":"..."}`.
