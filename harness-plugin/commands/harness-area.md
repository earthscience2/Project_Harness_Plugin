...
> **잠금 확인 (먼저):** `registry.json` 에 `"locked": true` 면 **상위 골격만 고정**이다(연구 프로필 등 — `skeleton` 의 root + 직속 자식들). 잠금은 **상위 골격에만** 적용:
> - **거부**: 새 **최상위** 영역 `add`(root 직속), 골격 영역의 `rm`·`split`·재배치(`parent`/`id`/`path` 변경) `move`. 이유 한 줄 — "상위 골격은 고정. 분열은 골격 영역 **하위**에만."
> - **허용**: 골격 영역 **하위**에 `add`/`split`(말단을 중간다리로 승격하며 자식 생성), 비골격 하위 영역의 `move`/`rm`. `done`/내용 편집·로그·일정도 허용.
