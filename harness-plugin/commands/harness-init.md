...
현재 저장소 루트에 **영역 하네스 골격**을 깐다. 기존 코드/파일은 건드리지 않는다(골격만 추가). 이후 새로 짤 코드는 처음부터 영역 안(`areas/<path>/src/`)에 만든다.
...
6. 사용자에게: 골격 완료. **이미 코드가 있으면 `/harness-adopt`** 로 영역에 수용(소스를 각 영역 `src/` 로 이동)하라고 안내. **빈 프로젝트면 새 코드를 처음부터 `areas/<path>/src/` 안에 만들라**고 안내(루트에 산포 금지) — 먼저 `/harness-area add` 로 말단을 만들고 그 `src/` 에 작성. 마인드맵은 `python -m http.server` → `http://localhost:8000/harness/`.

## 규칙
- init 자체는 골격 파일만 생성(기존 코드 무수정). 코드 수용/이동은 `/harness-adopt`·`/harness-area` 가 한다.
- 새 코드는 루트가 아니라 영역 `src/` 안에 만든다. 빈 프로젝트는 산포된 루트 코드 없이 영역 구조로 시작.
