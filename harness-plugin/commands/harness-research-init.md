...
2. **공유 인프라** 복사(없는 것만, 기존 보존): `${CLAUDE_PLUGIN_ROOT}/template/` 의 `check-registry.mjs` · `web/` · `Harness Ops Design System/` · `schedule/` · `automation/` · `logs/` · `reports/` (+ 있으면 `_ssot/`) 를 루트로 `cp`.
...
4. `GLOBAL.md`·`registry.json` 의 `updated`/`ts` 가 `TODO` 면 오늘 날짜로. `logs/log.jsonl` placeholder 도.
5. `node check-registry.mjs` → 통과(exit 0). 잠금 검사(skeleton 일치)·끊긴 참조 0 확인. (말단 `sources 비어 있음` 경고는 정상 — 데이터 매핑 전.)
6. `logs/log.jsonl` 에 한 줄: `{"ts":"<오늘>","actor":"ai","op":"area:add","target":"global","summary":"연구 하네스 초기화(고정 6영역)"}`.
7. 사용자에게: 골격 완료. **`GLOBAL.md` 의 「연구 목표」(목표·성공 기준·제약)를 먼저 채우라**고 안내. 마인드맵 `web/`, 보고서 `reports/index.html`.
